...
def loadDataSet(s):
        ...
        for line in frTrain.readlines():
                ...
                for i in range(c_len-1):
                        l[int(lineArr[i])]=1
                dataMat.append(l)
                labelMat.append(float(lineArr[c_len-1]))
...
def L2_sgd(feature,target,alpha = 0.001,Lambda = 20 ,iterateTimes = 1000):
	theta = np.zeros(feature.shape[1])#num of theta = num of feature atrribute
...
def L2_sgd_ColicTest():
		...
		for line in frtest.readlines():
				...
				linearr.append(l)			
				'''linearr=[]
				linearr.append([float(currline[i]) for i in range(len(currline)-1)])'''
				...
				testlabels.append(float(currline[len(currline)-1]))
				print("judge:",judge(np.array(linearr),trainweights),"truth:",currline[len(currline)-1])
				if int(judge(np.array(linearr),trainweights))!=int(currline[len(currline)-1]):
										errornum+=1
